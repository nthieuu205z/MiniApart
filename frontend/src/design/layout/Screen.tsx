...
export function MetaItem({ label, children }: { label: React.ReactNode; children: React.ReactNode }): React.ReactElement { return <div style={{ padding: 12, border: '1px solid var(--ma-border-default)', background: 'var(--ma-bg-sunken)' }}><dt>{label}</dt><dd style={{ margin: '5px 0 0' }}>{children}</dd></div> }
...
export function TableHeadCell({ children, align = 'left', colSpan }: { children: React.ReactNode; align?: 'left' | 'right'; colSpan?: number }): React.ReactElement { return <th colSpan={colSpan} style={{ padding: 12, borderBottom: '1px solid var(--ma-border-default)', textAlign: align, color: 'var(--ma-text-secondary)' }}>{children}</th> }
export function TableCell({ children, header = false, align = 'left', muted = false }: { children: React.ReactNode; header?: boolean; align?: 'left' | 'right'; muted?: boolean }): React.ReactElement { const Tag = header ? 'th' : 'td'; return <Tag style={{ padding: 12, borderBottom: '1px solid var(--ma-border-default)', textAlign: align, verticalAlign: 'top', overflowWrap: 'anywhere', color: muted ? 'var(--ma-text-secondary)' : undefined }}>{children}</Tag> }
