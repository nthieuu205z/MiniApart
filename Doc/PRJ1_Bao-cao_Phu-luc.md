...
## Phụ lục F — Lô phiếu yêu cầu thay đổi số 01
...
## Phụ lục G — Ma trận truy vết yêu cầu (đầy đủ)

Bản trích của ma trận truy vết đặt ở **mục 2.5.3**. Bản đầy đủ nằm ở tệp `PRJ1_Ma-tran-yeu-cau_Chung-cu-mini.xlsx` nộp kèm.

## Phụ lục H — Mã nguồn sơ đồ

Toàn bộ sơ đồ trong báo cáo đều lưu kèm mã nguồn, để sửa lại về sau mà không phải vẽ lại:

| Thư mục | Nội dung | Định dạng mã nguồn |
|---|---|---|
| `diagrams/` | Use case, hoạt động, luồng dữ liệu, trạng thái, và ERD phiên bản 1 | Mermaid (`.mmd`) |
| `diagrams-v2/` | ERD phiên bản 2, sơ đồ lớp, sơ đồ tuần tự | Mermaid (`.mmd`) |
| `diagrams-v2/` | Kiến trúc triển khai, phân rã module, vertical slice | Excalidraw (`.excalidraw`) |
